...
export default function GanadoCreateDialog({ fundo, razas }: GanadoCreateDialogProps) {
    const [open, setOpen] = React.useState(false);
    const [openDialog, setOpenDialog] = React.useState(false);

    const { data, setData, errors, processing, transform, reset, post } = useForm<Required<GanadoFormData>>({
        codigo: '',
        nombre: '',
        raza_id: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('ganados.store', fundo.id), {
            onSuccess: () => {
                toast.success('Ganado creado exitosamente');
                reset();
                setOpenDialog(false);
                setOpen(false);
            },
    ...
    return (
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Crear Nuevo Ganado</DialogTitle>
                    <DialogDescription>Completa los campos para registrar un nuevo ganado.</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-3">
                            ...
                            <div className="flex items-center gap-1">
                                <Select
                                    defaultValue={razas.length > 0 ? String(razas[0].id) : ''}
                                    required
                    ...
                    <Button type="submit" disabled={processing}>
                        {processing ? 'Guardando...' : 'Guardar Ganado'}
                    </Button>
                </form>
            </DialogContent>
        </Dialog>
